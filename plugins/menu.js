...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    text = text.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, 'g'), (_, name) => '' + replace[name])
     ...
     let res = await conn.prepareMessageFromContent(m.chat, { "orderMessage": { "orderId":"6288215463787", "itemCount": 2022, "message": text.trim(), "orderTitle": "L", "footerText": "© lolibot", "token": "AR6xBKbXZn0Xwmu76syd7rnxI+Rx87HfinVlW4lwXa6JA==", "thumbnail": tumbb, "surface": "CATALOG" } }, { quoted: m, sendEphemeral: true, contextInfo: { mentionedJid: [m.sender, ownernum] } })
     conn.relayWAMessage(res)
     } else if (chatp.menu == 2) {
     let menux =  await conn.prepareMessage(m.chat, tumb, MessageType.image, { quoted: m, thumbnail: tumb, contextInfo: { externalAdReply: { title: "あなたは私のすべてです", body: "L", previewType: "PHOTO", thumbnail: tumbb, sourceUrl: "" } } })
